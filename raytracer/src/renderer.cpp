...
renderer::renderer(vec3f const& background_color)
...
float renderer::compute_lighting(vec3f const& position, vec3f const& normal, vec3f const& view_direction, float specular_intensity) const
{
...
std::pair<sphere const*, float> renderer::compute_closest_sphere_intersection(vec3f const& origin, unit_vec3f const& direction, float near, float far) const
{
...
vec3f const renderer::compute_color_from_ray(vec3f const& origin, unit_vec3f const& direction, float near, float far, float recursion_depth) const
{
...
vec3f const renderer::compute_pixel_color(camera const& draw_camera, float u, float v) const
{
    auto const& near = draw_camera.get_near();
    auto const& far = draw_camera.get_far();
    vec3f const& ray_origin{vec3f::zero()};
    vec3f const& ray_direction = vec3f{u, v, near};
...
void renderer::draw_to_file(camera const& draw_camera) const
{
    ...
    std::vector<vec3f> canvas(static_cast<size_t>(width) * static_cast<size_t>(height), m_background_color);
...
vec3f const renderer::reflect_ray(vec3f const& ray, unit_vec3f const& around, float const* precomputed_dot) const
{
